...
# Keep the three discrimination measures plotted in Figures 7-9
...
for (survey_measure in discrimination_survey_measure_vector) {
  ...
  top_bottom_figure <- ggplot2::ggplot(top_bottom_plot_working_data, ggplot2::aes(x = firm)) +
    ...
    # Allow the angled firm names to render outside the panel on the common y-axis limits
  ...
  png(file.path(figures, paste0("top_bottom_firm_ratings_dual_axis_figures_", survey_measure, ".png")), width = 16, height = 7.3, units = "in", res = 300, bg = "white")
