...
# Survey measures plotted in draft Figures 10 and 11
draft_survey_measures <- c("pooled_favor_white", "pooled_favor_male", "conduct_favor_younger")
...
# Keep the survey measures plotted in the draft
firm_level_rating_estimates <- firm_level_rating_estimates |> dplyr::filter(outcome %in% draft_survey_measures)
...
# Keep the survey measures plotted in the draft
firm_level_robust_covariances <- firm_level_robust_covariances |> dplyr::filter(outcome %in% draft_survey_measures)
...
for (survey_measure_value in draft_survey_measures) {
...
for (survey_measure in draft_survey_measures) {
...
for (survey_measure in draft_survey_measures) {
  ...
  # Export the base figure used in the draft
  png(file.path(figures, paste0("industry_ratings_dual_axis_figures_within_", survey_measure, ".png")), width = 16, height = 7.3, units = "in", res = 300, bg = "white")
