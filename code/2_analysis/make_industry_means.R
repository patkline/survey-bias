# ------------------------------------------------------------------------------
# Add equal-weight industry means and demeaned firm outcomes
#
# Creates two outcomes per original outcome:
...
# - The optional entity_weight_col is copied to firm deviations and stores the
#   underlying firm count on industry means for the draft's decomposition table.
# ------------------------------------------------------------------------------
...
add_industry_means_to_results <- function(
  ...
  transform_result <- function(
    ...
    # Optional metadata column passthrough.
    if (!is.null(entity_weight_col)) {
      out_tbl[[entity_weight_col]] <- if (is.null(out_weight)) NA_real_ else as.numeric(out_weight)
    }
  ...
  for (set_name in which_sets) {
    ...
    for (model in model_names) {
      ...
      for (outcome in outcomes) {
        ...
        out_w_dm_eq <- NULL
        if (!is.null(entity_weight_col) && entity_weight_col %in% names(firm_ft)) {
          out_w_dm_eq <- firm_ft[[entity_weight_col]][match(out_ids_dm, firm_ft$entity_id)]
        }
