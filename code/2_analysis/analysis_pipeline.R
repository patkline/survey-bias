...
################################################################################
## Step 5c: Industry-characteristic EIV (industry covariate on LHS, beliefs on RHS)
################################################################################
  message("Building Industry-Characteristic EIV")

  # Bin-level covariates built in 1_data_build: EEO-1 employment shares + CPS residualized wage gaps/levels
  eeo1_shares  <- read.csv(file.path(dump, "industry_emp_share_by_demographic_eeo1.csv"),     stringsAsFactors = FALSE)
  cps_wagegaps <- read.csv(file.path(dump, "cps_industry_wage_gaps_levels_residualized.csv"), stringsAsFactors = FALSE)

  # Assemble one 19-row bin-level table (join on bin range + title; both come from the shared crosswalk)
  industry_chars <- dplyr::inner_join(
    eeo1_shares, cps_wagegaps,
    by = c("sic_two_digit_bin_aer", "sic_two_digit_bin_title_aer")
  )
  stopifnot(nrow(industry_chars) == 19)

  # Merge covariates onto firm coefficients by AER bin NAME (each firm inherits its bin's covariates).
  # Keying on the name, not aer_naics2, because the downward-extended bins (Freight 40-47, Banks 60-64)
  # have lower-bound numerics that differ from the firm map's aer_naics2 (42/61); the name is invariant.
  char_cols <- setdiff(names(industry_chars), c("sic_two_digit_bin_aer", "sic_two_digit_bin_title_aer"))
  coef_firm_chars <- coef_firm_wide %>%
    dplyr::left_join(
      dplyr::select(industry_chars, sic_two_digit_bin_title_aer, dplyr::all_of(char_cols)),
      by = c("aer_naics2_name" = "sic_two_digit_bin_title_aer")
    )
  stopifnot(sum(!is.na(coef_firm_chars$share_emp_black_all_jobs)) > 0)   # covariates landed on firms

  # Belief measures on the RHS (same belief set the existing EIV specs use)
  race_beliefs   <- c("FirmCont_favor_white", "conduct_favor_white", "pooled_favor_white",
                      "FirmCont_favor_white_ep", "conduct_favor_white_ep", "pooled_favor_white_ep",
                      "FirmSelective", "discretion")
  gender_beliefs <- c("FirmCont_favor_male", "conduct_favor_male", "pooled_favor_male",
                      "FirmCont_favor_male_ep", "conduct_favor_male_ep", "pooled_favor_male_ep",
                      "FirmSelective", "discretion")

  # Industry covariates on the LHS (race covariates against race beliefs, gender covariates against gender beliefs)
  race_chars   <- c("share_emp_black_all_jobs", "share_emp_black_mid_off_manager", "share_emp_black_front_line",
                    "wage_gap_black_coef", "wage_level_black_coef")
  gender_chars <- c("share_emp_female_all_jobs", "share_emp_female_mid_off_manager", "share_emp_female_front_line",
                    "wage_gap_sex_coef", "wage_level_sex_coef")

  # One spec per (covariate, belief) pair
  regs_chars <- list()
  for (lhs in race_chars)   for (rhs in race_beliefs)   regs_chars[[length(regs_chars) + 1L]] <- list(lhs = lhs, rhs = rhs)
  for (lhs in gender_chars) for (rhs in gender_beliefs) regs_chars[[length(regs_chars) + 1L]] <- list(lhs = lhs, rhs = rhs)

  # No FE: each covariate is constant within bin, so a naics2 FE would absorb it entirely
  write_eiv_sheet(
    output_dir, sheet_name = "EIV_industry_characteristics",
    regs          = regs_chars,
    coef_df_wide  = coef_firm_chars,
    noise_mats_97 = noise_mats_97,
    models        = models_to_run_eiv,
    id_col        = "entity_id",
    model_col     = "model",
  ...
  message("✅ Step 5c Complete. Output directory: ", output_dir)
