...
make_summary_table <- function(
  ...
  df_one <- df %>% group_by(ResponseId) %>% slice(1) %>% ungroup()
  ...
  counts_wide <- counts_wide %>%
    ...
    dplyr::select(variable, count_all, share_all, count_1, share_1, count_0, share_0) %>%
  ...
  n_row <- tibble(
    ...
    count_all = total_all, share_all = NA_real_,
    count_1   = total_1,   share_1   = NA_real_,
    count_0   = total_0,   share_0   = NA_real_
  )
  ...
  table_df_print <- table_df %>%
    ...
    dplyr::mutate(dplyr::across(-variable, ~ ifelse(is.na(.), "", as.character(.))))
